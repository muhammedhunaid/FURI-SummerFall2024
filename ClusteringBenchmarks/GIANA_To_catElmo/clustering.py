...
def HierarchicalClusteringCDR3(dM, flagL, n_clusters=2, linkage='ward', verbose=False):
    """
...
def EncodeRepertoire(inputfile, outdir, outfile='', exact=True, ST=3, thr_v=3.7, thr_s=3.5, VDict={}, Vgene=True, thr_iso=10, gap=-6, GPU=False, Mat=False, verbose=False, method='faiss', n_clusters=2, linkage='ward'):
    ...
    for kk in LDu:
        ...
        if method == 'hierarchical':
            Cls = HierarchicalClusteringCDR3(dM, flagL, n_clusters=n_clusters, linkage=linkage, verbose=verbose)
        else:
...
''')
    ...
    parser.add_option("--method", dest="method", default='faiss', help="Clustering method to use ('faiss' or 'hierarchical').")
    parser.add_option("--n_clusters", dest="n_clusters", default=2, help="Number of clusters for hierarchical clustering.")
    parser.add_option("--linkage", dest="linkage", default='ward', help="Linkage criterion for hierarchical clustering ('ward', 'complete', 'average', 'single').")
    return parser.parse_args()

def main():
    (opt,_)=CommandLineParser()
    cutoff=float(opt.thr)
    OutDir = './results'  # or any other writable directory
